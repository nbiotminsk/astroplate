...
7. Отправка POST-запроса на вебхук хостинга HostFly для обновления продакшена.
...
В конце успешного выполнения CI/CD пайплайна на GitHub Actions автоматически отправляется POST-запрос на вебхук вашего хостинга HostFly для разворачивания (pull) собранной ветки `deploy` на «живом» сервере.

В случае если шаг вебхука на GitHub Actions завершился неудачей (например, из-за сетевого тайм-аута, код ошибки 28 в curl), вы должны отправить этот запрос вручную с вашего локального терминала:
...
# 7. Запуск вебхука (вручную при необходимости)
curl -X POST --max-time 15 "https://by4010.hb.by:8443/modules/git/public/web-hook.php?uuid=545844a5-6f56-2a7d-37aa-0a92eb58d127"
```
...
- Вебхук успешно отработал с HTTP 204.
